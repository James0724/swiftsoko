...
export function Navbar() {
  ...
  useEffect(() => {
    const HIDE_AFTER_PX = 60; // distance that must accumulate in one direction before toggling
    const TOP_OFFSET_PX = 80; // always show the header near the very top

    let lastY = window.scrollY;
    let anchorY = lastY; // where the current directional run started
    let direction = 0; // -1 up, 1 down, 0 unknown
    let ticking = false;

    // Hysteresis-based: a toggle only fires once scroll has moved
    // HIDE_AFTER_PX in one direction since the last reversal. A trackpad's
    // momentum/rubber-band wobble after a "slight" scroll is far smaller
    // than that, so it can no longer flip the header back and forth.
    const evaluate = () => {
      ...
      const newDirection = y > lastY ? 1 : y < lastY ? -1 : direction;
      if (newDirection !== direction) {
        anchorY = lastY;
        direction = newDirection;
      }

      const movement = y - anchorY;
      if (direction === 1 && movement > HIDE_AFTER_PX) {
        setIsHeaderHidden(true);
        anchorY = y;
      } else if (direction === -1 && movement < -HIDE_AFTER_PX) {
        setIsHeaderHidden(false);
        anchorY = y;
      }
  ...
  return (
    <nav className="sticky top-0 z-50 w-full">
      {/* ── LAYERS 1 & 2 collapse together on scroll-down, reappear on scroll-up ── */}
      <div
        className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${
          isHeaderHidden ? "grid-rows-[0fr]" : "grid-rows-[1fr]"
        }`}
      >
        <div className="overflow-hidden">
          {/* ── LAYER 1: ANNOUNCEMENT TICKER ── */}
          <div className="bg-indigo-950 overflow-hidden py-1 border-b-2 border-indigo-800">
          ...
          <div className="bg-white border-b-2 border-black">
            <div className="max-w-7xl mx-auto px-4 h-12 sm:h-14 flex items-center gap-4">
              {/* LOGO */}
              <Link
                href="/"
              ...
              >
                <Input
                  ...
                  className="rounded-none border-2 border-black h-9 font-bold text-sm placeholder:text-gray-400 placeholder:font-bold placeholder:uppercase placeholder:text-xs focus-visible:ring-0 focus-visible:border-indigo-600 pr-14"
                  placeholder="Search products, brands & categories..."
                />
                <button
                  type="submit"
                  aria-label="Search"
                  className="absolute right-0 top-0 h-9 w-11 bg-indigo-950 border-l-2 border-black flex items-center justify-center hover:bg-indigo-800 transition-colors"
                >
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 ml-auto">
                ...
                <button
                  ...
                  className="md:hidden w-9 h-9 flex items-center justify-center border-2 border-black hover:bg-yellow-300 transition-colors"
                >
                  {isSearchOpen ? (
                ...
                <Link
                  href="/wishlist"
                  aria-label="Wishlist"
                  className="hidden sm:flex w-9 h-9 items-center justify-center border-2 border-black hover:bg-yellow-300 group transition-colors"
                >
                ...
                <button
                  onClick={openCart}
                  aria-label="Cart"
                  className="flex items-center gap-1.5 h-9 px-3 border-2 border-black bg-yellow-300 font-black uppercase text-xs tracking-widest hover:bg-yellow-400 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.75 hover:translate-y-0.75 transition-all"
                >
                ...
                {session ? (
                  <DropdownMenu modal={false}>
                    <DropdownMenuTrigger asChild>
                      <button className="hidden md:flex items-center gap-2 h-9 px-3 border-2 border-black bg-indigo-950 text-white font-black uppercase text-xs tracking-widest hover:bg-indigo-800 transition-colors shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.75 hover:translate-y-0.75">
                  ...
                  <div className="hidden md:flex items-center gap-2">
                    <button
                      ...
                      className="h-9 px-4 border-2 border-black font-black uppercase text-xs tracking-widest hover:bg-black hover:text-white transition-colors"
                    >
                      Login
                    </button>
                    <button
                      onClick={() => router.push("/login")}
                      className="h-9 px-4 border-2 border-black bg-yellow-300 font-black uppercase text-xs tracking-widest shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.75 hover:translate-y-0.75 transition-all"
                    >
                      Sign Up
                    </button>
                  </div>
            ...
            {/* Mobile search bar */}
            {isSearchOpen && (
              <div className="md:hidden px-4 pb-2 border-t border-gray-200">
                <form onSubmit={handleSearch} className="relative flex">
                  <Input
                    autoFocus
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="rounded-none border-2 border-black h-9 font-bold text-sm placeholder:font-bold placeholder:uppercase placeholder:text-xs focus-visible:ring-0 pr-12"
                    placeholder="Search..."
                  />
                  <button
                    type="submit"
                    className="absolute right-0 top-0 h-9 w-11 bg-indigo-950 border-l-2 border-black flex items-center justify-center"
                  >
                    <Search className="h-4 w-4 text-white stroke-[3px]" />
                  </button>
                </form>
      ...
      >
        <div className="max-w-7xl mx-auto  flex items-stretch h-9">
          ...
          {/* Action CTAs */}
          <div className="hidden lg:flex items-stretch ml-auto border-x-2 border-black">
            <a
              href="https://wa.me/254700000000"
              target="_blank"
              rel="noopener noreferrer"
        ...
        {isMegaOpen && (
          <div
            ...
            className="absolute left-0 right-0 bg-white border-t-2 border-x-2 border-b-4 border-black shadow-[0_6px_0px_0px_rgba(0,0,0,1)] z-40"
          >
